...
const Phone: React.FC = props => {
    return (
        <svg
            ...
            <rect x="14.09" y="15.04" width="211.7" height="449.76" fill="#fff"/>
            <SnakePlatform x={14.09} y={15.04} width={211.7} height={449.76}/>
...
const SnakePlatform: React.FC<SnakePlatformProps> = ({ x, y, width, height }) => {
    // create snake 
    var size = 25
    ...
    return (
        <svg xmlns="http://www.w3.org/2000/svg" x={x} y={y} width={width} height={height} fill='#000'>
            <Snake width={25} borderWidth={10} color={SECONDARY} initialX={186}/>
            <Snake width={25} borderWidth={10} color={TERTIARY} />
        </svg>
    )
}
...
const Snake: React.FC<SnakeProps> = ({ color, borderWidth, initialSize, initialX, width }) => {
    // width, borderWidth, color
    let height = width
    let moveDistane = width
    initialSize = initialSize | 6
    ...
    const [snakeState, updateSnakeState] = useState({
        direction: 3,
        snakeBody: [{x: 0, y: 0}]
    })

    useKeypress('ArrowUp', () => {
        updateSnakeState(prevSnakeState => ({
            ...prevSnakeState, 
            direction: 1
        }))
    });
    ...
    //     updateSnakeState(prevSnakeState => {
    //         let snakeBody = prevSnakeState.snakeBody
    //         let head = snakeBody[0]
    //         head.y += ((moveDistane * deltaTime * 0.001))
    //         snakeBody[0] = head
    ...
    return (
        <g>
            {/* {snake} */}
            { <SnakeBlock x={snakeState.snakeBody[0].x} y={snakeState.snakeBody[0].y} width={25} height={25} color={color} /> }
        </g>
    )
}
...
const SnakeBlock: React.FC<SnakeBlockProps> = ({ x, y, width, height, color }) => {
    return (
        <rect x={x} y={y} width={width} height={height} fill={color} stroke={color} style={{ transition: 'all .3s linear' }}></rect>
    )
}
